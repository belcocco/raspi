...
INT11 = 11      # define GPIO pin number for interrupt 555 (pin3) con GPIO 11
INT4 = 4      # define GPIO pin number for interrupt CD4017 (pin12) con GPIO 4 
lastTime = time.time()
dMax1 = 0;
dMin1 = 2;
dSum1 = 0;
...
#Definizione della callback per impulsi in uscita da 4017 su pin 12 (collegato a GPIO 4)
...
RPIO.add_interrupt_callback(INT11, gpio_callback_1, edge='rising', pull_up_down=RPIO.PUD_UP)#, debounce_timeout_ms=100)
RPIO.add_interrupt_callback(INT4, gpio_callback_2, edge='rising', pull_up_down=RPIO.PUD_UP)#, debounce_timeout_ms=100)
...
try:
	while (1):
		...
		print("Time555: %s Avg: %08.6f Min: %08.6f Max: %08.6f Total: %d" % (dstr1, dAvg1, dMin1, dMax1, dCount1))
   		...
   		if (dCount2 > 0):
      			dAvg2 = dSum2 / dCount2
		print("Time4017: %s Avg: %08.6f Min: %08.6f Max: %08.6f Total: %d" % (dstr2, dAvg2, dMin2, dMax2, dCount2))

   		time.sleep(10)  #1) Stampa 1 volta ogni secondo 
